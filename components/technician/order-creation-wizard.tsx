...
import { User, Wrench, CheckCircle, ArrowRight, ArrowLeft, Search, Monitor, Plus, Info, Lightbulb } from "lucide-react"
...
interface TechnicianOrderCreationWizardProps {
  isOpen: boolean
  onClose: () => void
...
export function TechnicianOrderCreationWizard({
  isOpen,
  onClose,
  onOrderCreated,
  technicianId,
}: TechnicianOrderCreationWizardProps) {
  const [currentStep, setCurrentStep] = useState<Step>("client-type")
  const [isLoading, setIsLoading] = useState(false)
  const [errors, setErrors] = useState<Record<string, string>>({})

  const [authenticatedTechnicianId, setAuthenticatedTechnicianId] = useState<string | null>(null)
  ...
  useEffect(() => {
    const getAuthenticatedUser = async () => {
      try {
        ...
        if (user) {
          console.log("[v0] Authenticated technician ID:", user.id)
          setAuthenticatedTechnicianId(user.id)
        } else {
          console.error("[v0] No authenticated user found")
          toast.error("No se encontró un usuario autenticado")
        }
      } catch (error) {
        console.error("[v0] Error in getAuthenticatedUser:", error)
  ...
  const createOrder = async () => {
    ...
    const technicianIdToUse = authenticatedTechnicianId || technicianId

    if (!technicianIdToUse) {
      toast.error("No se pudo identificar al técnico. Por favor, inicia sesión nuevamente.")
      return
    }
    ...
    try {
      ...
      console.log("[v0] Creating order with authenticated technician ID:", technicianIdToUse)
      ...
      toast.success(`¡Orden ${orderNumber} creada exitosamente y asignada a ti!`)
    ...
    switch (currentStep) {
      ...
      case "order-details":
        return {
          ...
          content:
            "Una descripción detallada del problema ayuda en la preparación. El costo estimado es opcional pero útil para el cliente. La orden se asignará automáticamente a ti.",
        }
      default:
        return null
    }
          ...
          <div className="space-y-4">
            ...
            <Alert className="border-blue-200 bg-blue-50 dark:bg-blue-900/20">
              ...
              <AlertDescription className="text-blue-800 dark:text-blue-200">
                Esta orden se asignará automáticamente a ti como técnico responsable.
              </AlertDescription>
            </Alert>
          </div>
        )
          ...
          <div className="text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                La orden ha sido registrada y asignada a ti. Puedes comenzar a trabajar en ella de inmediato.
              </p>
            </div>
      ...
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-center space-x-2">
            {["client-type", "equipment-type", "order-details", "confirmation"].map((step, index) => {
              const stepMap = {
              ...
              const isCompleted =
                ...
                (step === "order-details" && currentStep === "confirmation")
              ...
              return (
                <div key={step} className="flex items-center">
                  ...
                  {index < 3 && <div className={`w-8 h-0.5 mx-2 ${isCompleted ? "bg-green-500" : "bg-muted"}`} />}
                </div>
              )
            })}
          </div>
          ...
          {currentStep !== "confirmation" && (
            <div className="flex justify-between">
              ...
              <Button onClick={nextStep} disabled={!canProceed() || isLoading}>
                {isLoading ? "Procesando..." : currentStep === "order-details" ? "Crear Orden" : "Siguiente"}
                {!isLoading && currentStep !== "order-details" && <ArrowRight className="w-4 h-4 ml-2" />}
              </Button>
            </div>
          )}
