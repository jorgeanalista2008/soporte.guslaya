...
import type { Equipment, UpdateEquipmentData } from "@/types/equipment"
...
export function EditEquipmentDialog({ equipment, open, onOpenChange, onSuccess }: EditEquipmentDialogProps) {
  ...
  const [formData, setFormData] = useState<UpdateEquipmentData>({
    equipment_type: "PC",
    brand: "",
    model: "",
    equipment_subtype: "",
  ...
  useEffect(() => {
    if (equipment && open) {
      ...
      if (equipment.equipment_components) {
        setComponents(
          equipment.equipment_components.map((comp) => ({
            id: comp.id,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Modifique la información del equipo seleccionado.</DialogDescription>
        </DialogHeader>
