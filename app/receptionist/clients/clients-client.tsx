...
import { Plus, Users, UserPlus, Building, TrendingUp, Monitor } from "lucide-react"
...
import { ClientEquipmentsDialog } from "@/components/clients/client-equipments-dialog"
...
export function ClientsPageClient({ clients: initialClients, stats }: ClientsPageClientProps) {
  ...
  const handleManageEquipments = (client: Client) => {
    setSelectedClient(client)
  ...
  const statsIcons = [
    <Users key="total" className="w-6 h-6" />,
    <UserPlus key="active" className="w-6 h-6" />,
    <Users key="inactive" className="w-6 h-6" />,
    <Building key="orders" className="w-6 h-6" />,
    <Monitor key="equipments" className="w-6 h-6" />,
    <TrendingUp key="new" className="w-6 h-6" />,
  ]
  ...
  const getRecentClientsByCategory = (category: string) => {
    ...
    switch (category) {
      ...
      case "withEquipments":
        return sortedClients.filter((client) => (client._count?.equipments || 0) > 0).slice(0, 5)
      case "newThisMonth":
        const now = new Date()
  ...
  const getStatsDialogContent = (index: number) => {
    const categories = ["total", "active", "inactive", "withOrders", "withEquipments", "newThisMonth"]
    const titles = [
      "Total Clientes",
      "Clientes Activos",
      "Clientes Inactivos",
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex gap-2 text-xs text-muted-foreground">
                        {client._count?.service_orders !== undefined && (
                          <span>{client._count.service_orders} órdenes</span>
                        )}
                        {client._count?.equipments !== undefined && <span>{client._count.equipments} equipos</span>}
                      </div>
                    </div>
                  </div>
                ))
  ...
  return (
    <div className="w-full space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">Gestión de Clientes y Equipos</h1>
          <p className="text-muted-foreground">Administra clientes y sus equipos asociados</p>
        </div>
      ...
      {/* Stats Cards - Agregada tarjeta de equipos */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
        ...
        <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setSelectedStatsCard(4)}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Con Equipos</CardTitle>
            {statsIcons[4]}
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">{stats.withEquipments}</div>
          </CardContent>
        </Card>
        <Card className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setSelectedStatsCard(5)}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <ClientEquipmentsDialog
        client={selectedClient}
        open={showEquipmentsDialog}
        onOpenChange={setShowEquipmentsDialog}
        onSuccess={() => {
          setShowEquipmentsDialog(false)
...
export { ClientsPageClient as ClientsClient }
